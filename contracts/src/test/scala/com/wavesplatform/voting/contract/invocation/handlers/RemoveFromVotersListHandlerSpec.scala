...
class RemoveFromVotersListHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  "all validations passed" in {
    ...
    val result      = RemoveFromVotersListHandler.call(contractTransaction, state)
    result shouldBe Right(Seq(DataEntry(StateKey.VotersListRemove(contractTransaction.id), toJsonStr(userIdHashes))))
  }
