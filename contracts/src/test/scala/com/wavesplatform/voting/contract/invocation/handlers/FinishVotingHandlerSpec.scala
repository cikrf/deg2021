...
import com.wavesplatform.voting.contract.{VotingBase, VotingStatus, _}
  ...
  "check status halted" in {
    val contractState: ContractKeysResponse =
      ContractKeysResponse(
        serversDE +: Seq(
          votingBase.copy(dateStart = Some(Instant.now().plus(2, ChronoUnit.DAYS).truncateToSeconds())).toDataEntry))
    val callRes = FinishVotingHandler.call(contractTransaction, contractState)
    callRes shouldBe 'right
