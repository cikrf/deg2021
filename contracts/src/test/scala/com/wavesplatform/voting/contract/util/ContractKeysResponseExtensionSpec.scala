...
import com.wavesplatform.voting.contract.{toJsonStr, Dim, StateKey, VotingBase, VotingStatus}
...
class ContractKeysResponseExtensionSpec extends AnyFreeSpec with Matchers {
  ...
  private val servers              = Seq("pubKey1", "pubKey2")
  private val serversDE            = common.DataEntry(StateKey.Servers, toJsonStr(servers))
  private val defaultDataEntries   = Seq(votingBase.toDataEntry, serversDE)
  private val contractKeysResponse = ContractKeysResponse(entries = defaultDataEntries)

  s"parse ${StateKey.VotingBase.entryName} from ContractKeysResponse.entries" in {
    ...
    parsedVotingBase.right.get shouldBe votingBase
      .copy(
        dateStart = votingBase.dateStart.map(_.truncateToSeconds()),
        dateEnd = votingBase.dateEnd.map(_.truncateToSeconds()))
  }

  s"parse ${StateKey.Servers.entryName} from ContractKeysResponse.entries" in {
    val parsedServers = contractKeysResponse.getServers
    parsedServers shouldBe 'right
    parsedServers.right.get should contain theSameElementsAs servers
  }
}
