...
class ResultHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  private val contractTransaction = ContractTransaction(
    ...
    params = Seq(
      DataEntry(key = ParamKey.Results, value = DataEntry.Value.StringValue(resultsValue))
    ),
    fee = 22L,
    version = 2,
  ...
  "all validations passed" in {
    ...
    callRes.right.get should contain theSameElementsAs Seq(
      DataEntry(StateKey.Results, DataEntry.Value.StringValue(resultsValue)))
  }
  ...
  "there is no 'results' in contractTransaction" in {
    ...
    val contractTransactionWithoutMainKey =
      contractTransaction.replaceDataEntryParam(ParamKey.Results, DataEntry.Value.Empty)
    val callRes = ResultsHandler.call(contractTransactionWithoutMainKey, contractState)
    callRes shouldBe 'left
    callRes.left.get shouldBe VotingError.RequiredParamIsMissing(ParamKey.Results)
  }
