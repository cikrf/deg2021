...
class AddToVotersListHandlerSpec extends AnyFreeSpec with Matchers {
  ...
  "all validations passed" in {
    ...
    val result      = AddToVotersListHandler.call(contractTransaction, state)
    result shouldBe Right(Seq(DataEntry(StateKey.VotersListAdd(contractTransaction.id), toJsonStr(userIdHashes))))
  }

  "should fail when sender is not the registrator" in {
    val otherRegistrator = DataEntry(StateKey.VotersListRegistrator, StringValue("stop"))
