...
/**
  ...
  * Запись:
  *
  * Значение userIdHashes на ключ VOTERS_LIST_ADD_<txId>
  */
object AddToVotersListHandler extends CallHandler {
  ...
  override def call(tx: ContractTransaction, state: ContractKeysResponse): Either[VotingError, Seq[DataEntry]] = {
    ...
    } yield DataEntry(StateKey.VotersListAdd(tx.id), toJsonStr(userIdHashes)) :: Nil
  }

  override def operationName: String = "addtovoterslist"
}
